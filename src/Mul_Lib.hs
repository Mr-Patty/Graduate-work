...
-- extendVec a _ = a
nullMul :: BinaryVector -> BinaryVector -> BinaryVector
nullMul _ _ = Vec.empty
...
naiveMul vecA vecB =
  let
    ...
    lenMax = max (Vec.length vecA) (Vec.length vecB)
    resVec = Vec.replicate (2 * lenMax) 0
    listVec = mulVec vecA vecB 0
  in
    List.foldl' add resVec listVec
    -- finalizeVec $ List.foldl' foldFunc resVec listVec

add :: BinaryVector -> BinaryVector -> BinaryVector
add a b = Vec.zipWith (+) a b
...
mulVec vecA vecB n =
  case Vec.null vecA of
    False ->
      ...
      in
        (resNul Vec.++ (Vec.map (\x -> x * a) vecB)) : mulVec vecAEnd vecB (n + 1)
    _ -> []
